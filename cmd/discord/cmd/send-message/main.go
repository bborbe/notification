...
import (
	...
	"github.com/bborbe/notification/cmd/discord/pkg"
...
func (a *application) Run(ctx context.Context, sentryClient libsentry.Client) error {
	...
	messageSender := pkg.NewMessageSender(discordSession)

	channelIDFinder := pkg.NewChannelIDFinder(
		pkg.NewChannelStreamer(
			discordSession,
			pkg.ServerID(a.DiscordServerID),
		),
	)
	channels, err := channelIDFinder.FindByName(ctx, "test")
	if err != nil {
