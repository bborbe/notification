...
.PHONY: ensure
	...
	go mod verify
	rm -rf vendor
	# go mod tidy -e removes bazelbuild from require due to proxy package-resolution issues
	# (bborbe/kv -> osv-scanner -> osv-scalibr chain); restore it so errcheck can type-check.
	# Remove this line once bborbe/kv is migrated from tools.go to tools.env.
	go mod edit -require=github.com/bazelbuild/buildtools@v0.0.0-20250826111327-4006b543a694

.PHONY: format
format:
	find . -type f -name 'go.mod' -not -path './vendor/*' -exec go run github.com/shoenig/go-modtool@$(GO_MODTOOL_VERSION) -w fmt "{}" \;
	go run github.com/incu6us/goimports-reviser/v3@$(GOIMPORTS_REVISER_VERSION) -project-name github.com/bborbe/http -format -excludes vendor ./...
...
VULNCHECK_IGNORE ?= GO-2026-4923 GO-2026-4514 GO-2022-0470 GO-2026-4772 GO-2026-4771 GO-2026-4883 GO-2026-4887
