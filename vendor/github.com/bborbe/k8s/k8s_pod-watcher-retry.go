...
// NewPodWatcherRetry wraps a PodWatcher with automatic retry logic.
...
type podWatcherRetry struct {
	podWatcher PodWatcher
	waiter     libtime.WaiterDuration
	duration   libtime.Duration
}

func (s *podWatcherRetry) Watch(ctx context.Context) error {
	glog.V(2).Infof("pod watcher retry started")
	...
	for {
		...
		case <-ctx.Done():
			return ctx.Err()
		default:
			if err := s.podWatcher.Watch(ctx); err != nil {
				if errors.Is(err, context.Canceled) || errors.Is(err, context.DeadlineExceeded) {
					return errors.Wrapf(ctx, err, "pod watcher watch failed")
				}
