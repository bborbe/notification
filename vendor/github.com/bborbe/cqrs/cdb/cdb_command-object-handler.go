...
//counterfeiter:generate -o ../mocks/cdb-command-object-handler.go --fake-name CDBCommandObjectHandler . CommandObjectHandler
...
func (o CommandObjectHandlerList) Handle(ctx context.Context, commandObject CommandObject) error {
	for _, oo := range o {
		...
		case <-ctx.Done():
			return ctx.Err()
		default:
			if err := oo.Handle(ctx, commandObject); err != nil {
...
func (c *commandObjectHandler) Handle(ctx context.Context, commandObject CommandObject) error {
	...
	if _, _, err := commandObjectExecutor.HandleCommand(ctx, commandObject); err != nil {
